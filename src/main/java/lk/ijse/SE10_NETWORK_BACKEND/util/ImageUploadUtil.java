...
public class ImageUploadUtil {
    private static final String path = "C:\\Users\\Shadow\\Downloads\\SE10-NETWORK-BACKEND(COPY)\\src\\main\\resources\\static\\";
    ...
    public static void saveFile(Long userId, String type, MultipartFile multipartFile) throws IOException {
        ...
        if (!staticDir.exists()) {
            staticDir.mkdirs();
            log.info("Created static directory: {}", path);
        }
        String uploadDir = path + userId;
        Path uploadPath = Paths.get(uploadDir);
        if (!Files.exists(uploadPath)) {
            Files.createDirectories(uploadPath);
            log.info("Created directory for user {}: {}", userId, uploadDir);
        }
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(uploadPath, fileNameWithoutExtension + ".*")) {
            for (Path existingFile : stream) {
                ...
                log.info("Deleted existing file: {}", existingFile.getFileName());
            }
        } catch (IOException ioException) {
            log.error("Failed to delete existing image file: {}", fileNameWithoutExtension, ioException);
        ...
        try (InputStream inputStream = multipartFile.getInputStream()) {
            ...
            log.error("Failed to save image file: {}", fileName, ioException);
    ...
    public static String getProfileImage(Long userId) {
        log.debug("Searching for profile photo for user {}", userId);
        try {
            for (String format : formats) {
                File profilePic = new File(path + userId + "\\" + userId + "_profile_photo" + format);
                if (profilePic.exists()) {
                    log.info("Profile photo found for user {}", userId);
                    return encodeFileToBase64(profilePic);
                }
            }
        } catch (IOException e) {
            log.error("Failed to retrieve profile photo for user {}", userId, e);
        }
        log.warn("No profile photo found for user {}", userId);
        return null;
    }
    /**
    ...
    public static String getCoverImage(Long userId) {
        log.debug("Searching for cover photo for user {}", userId);
        try {
            for (String format : formats) {
                File coverPic = new File(path + userId + "\\" + userId + "_cover_photo" + format);
                if (coverPic.exists()) {
                    log.info("Cover photo found for user {}", userId);
                    return encodeFileToBase64(coverPic);
                }
            }
        } catch (IOException e) {
            log.error("Failed to retrieve cover photo for user {}", userId, e);
        }
        log.warn("No cover photo found for user {}", userId);
        return null;
    }
    /**
    ...
    public static void deleteFile(Long userId, String type) throws IOException {
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(dirPath, fileNameWithoutExtension + ".*")) {
            ...
            if (!fileDeleted) {
                log.warn("File not found: {}", fileNameWithoutExtension);
                throw new IOException("File not found: " + fileNameWithoutExtension);
            }
        } catch (IOException ioException) {
            log.error("Failed to delete image file: {}", fileNameWithoutExtension, ioException);
    ...
    private static String encodeFileToBase64(File file) throws IOException {
        ...
        log.debug("File encoded to Base64: {}", file.getName());
